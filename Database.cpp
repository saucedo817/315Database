...
namespace Database {
	...
	string& MyRecord::operator[](std::size_t idx) {
		return mVector[idx];
	}
	...
	void makeKey(Table* tableName, std::string keyName) {
		int index;
		...
		for (int i = 0; i < tableName->numRow; ++i) {
			//attrString.push_back(tableName.at(i).at(index));
		}
	...
	std::vector<std::vector<std::string>> crossJoin(std::vector<std::vector<std::string>> table1, std::vector<std::vector<std::string>> table2) {
		//int addSize = table2.numRow; //not sure why table 1 and table 2 are being inputted as 2D vectors. Should be tables to allow for function calls
		//for (int i = 0; i < addSize; i++) {
			//table1.insertRecord(table1, table2.at(i));
			//numRow++;
		//}
		//delete table 2
	}
	...
	int count(Table* tableName, std::string attribute) {
		...
		int counter;
		for (int i = 0; i < tableName->attributeList.size(); i++) {
			if (tableName->attributeList[i] == attribute) {
				int index = i;
			}
		}
		for (int i = 0; i < tableName->numRow; ++i) {
